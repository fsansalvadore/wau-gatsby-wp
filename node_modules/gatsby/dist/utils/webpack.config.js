...
module.exports = async (program, directory, suppliedStage, port, {
    ...
    switch (stage) {
      case `develop`:
        return {
          ...
          commons: [process.env.GATSBY_HOT_LOADER !== `fast-refresh` && `${require.resolve(`webpack-hot-middleware/client`)}?path=${getHmrPath()}`, directoryPath(`.cache/app`)].filter(Boolean)
        };
  ...
  function getDevtool() {
    switch (stage) {
      case `develop`:
        return process.env.GATSBY_HOT_LOADER !== `fast-refresh` ? `cheap-module-source-map` : `eval-cheap-module-source-map`;
  ...
  if (stage === `build-html` || stage === `develop-html`) {
    ...
    config.externals = [function (context, request, callback) {
      if (stage === `develop-html` && isCI() && process.env.GATSBY_EXPERIMENTAL_DEV_SSR) {
        if (request === `react`) {
          callback(null, `react/cjs/react.production.min.js`);
          return;
        } else if (request === `react-dom/server`) {
          callback(null, `react-dom/cjs/react-dom-server.node.production.min.js`);
          return;
        }
      }
