                ...
                <Collapse accordion>
                    {
                        list.map((item, index) => (
                            <Panel
                                ...
                                key={index}
                                // showArrow={false}
                                tw="border-0 border-t border-solid border-white py-4 text-2xl md:text-4xl font-bold cursor-pointer"
                            >
                                <div tw="py-4 text-lg">
                                    <p tw="mb-8">{item.expertiseACF.anteprima}</p>
                                    <Button
                                        as={Link}
                                        to={lang === "it" ? `/expertise/${item.slug}` : `/en/expertise/${item.slug}`}
                                    >{ lang === "it" ? "Approfondisci" : "Explore" }</Button>
                                </div>
                            </Panel>
                        ))
                    }
